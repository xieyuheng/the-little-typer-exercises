...
``` cicada
flavored_lentils(flavor: String): Pair(String, String) {
  return cons(@elab flavor, "lentils")
}

garlic_flavored_lentils: Pair(String, String) =
  flavored_lentils(@elab "garlic")

id(x: String): String { return x }

T: Type = Pair(String, (String) -> String)
...
the(
  Equal(
    (Nat) -> Nat,
    (y) => car(the(Pair(Nat, Nat), cons(y, y))),
    (x) => x),
  refl)
