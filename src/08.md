...
``` cicada
function add_1_add1_equal(n: Nat): Equal(Nat, add(1, n), add1(n)) {
  return refl
}
```
...
function incr_add1_equal_by_cong_opts(n: Nat): Equal(Nat, incr(n), add1(n)) {
  return nat_ind(
    n,
    incr_add1_equal_motive,
    incr_add1_equal_base,
...
function incr_add1_equal_step_by_cong_explicit(
  ...
  return cong_explicit(Nat, incr(prev), add1(prev), almost, Nat, add(1))
}

function incr_add1_equal_by_cong_explicit(n: Nat): Equal(Nat, incr(n), add1(n)) {
  return nat_ind(
    n,
    incr_add1_equal_motive,
    incr_add1_equal_base,
