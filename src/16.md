...
zero_equal_decide(zero)
zero_equal_decide(one)
zero_equal_decide(two)
zero_equal_decide(three)
```
...
function add1_equal_decide(
  ...
  return induction (eq_or_not) {
    (_) => Decide(Equal(Nat, Nat.add1(x), Nat.add1(y)))
    case inl(equal) => {
...
nat_equal_decide(zero, zero)
nat_equal_decide(one, zero)
nat_equal_decide(zero, one)
nat_equal_decide(six, four)
```
